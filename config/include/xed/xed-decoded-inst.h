...
typedef struct xed_decoded_inst_s  {
    ...
    /// operands all have the form xed_decoded_inst_get_operand().
    xed_operand_storage_t _operands;

#if defined(XED_ENCODER)
    /// Used for encode operand ordering. Not set by decoder.
    xed_uint8_t _n_operand_order; 
    xed_uint8_t _operand_order[XED_ENCODE_ORDER_MAX_OPERANDS];
#endif

    /// when we decode, we set the _operands to zero so that any missing
    /// fields are zero.
